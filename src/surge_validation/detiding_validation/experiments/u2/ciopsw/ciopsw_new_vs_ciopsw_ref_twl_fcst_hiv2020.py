...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("CIOPSW (FCST-REF, TWL)",
            next((inp_data_root / "ciopsw").rglob(f"data_for_scoring_*ciopsw*_ref_*_twl_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSW (FCST-NEW, TWL)",
            next((inp_data_root / "ciopsw").rglob(f"data_for_scoring_*ciopsw*_new_*_twl_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "CIOPSW_NEW_vs_CIOPSE_REF_FCST_HIV2020_TWL"
