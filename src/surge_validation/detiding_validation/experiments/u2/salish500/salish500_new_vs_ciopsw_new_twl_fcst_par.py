...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/u2/salish/par/{label}")

    exp_id_to_path = OrderedDict([
        ("SALISH500 (FCST-REF, TWL)",
            next((inp_data_root / "ciopsw/ss500/par").rglob(f"data_for_scoring_*salish500*_ref_*par_twl_{st_s}_{en_s}/surge*.dat"))),
        ("SALISH500 (FCST-NEW, TWL)",# data_for_scoring_salish500_new_par_blend_surge_2021080500_2021092600
            next((inp_data_root / "ciopsw/ss500/par").rglob(f"data_for_scoring_*salish500*_new_*par_twl_*{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "SALISH500_NEW_vs_SALISH500_REF_FCST_PAR_TWL"
