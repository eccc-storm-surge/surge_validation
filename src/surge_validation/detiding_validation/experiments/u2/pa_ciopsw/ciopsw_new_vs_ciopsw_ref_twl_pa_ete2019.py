...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("CIOPSW (PA-REF, TWL)",
            next(inp_data_root.rglob(f"data_for_scoring_*ciopsw*_ref_*_twl_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSW (PA-NEW, TWL)",
            next(inp_data_root.rglob(f"data_for_scoring_*ciopsw*_new_*_twl_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "CIOPSW_NEW_vs_CIOPSW_REF_PA_ETE2019_TWL"
