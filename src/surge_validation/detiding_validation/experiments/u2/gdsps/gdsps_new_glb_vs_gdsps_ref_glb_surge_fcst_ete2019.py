...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-REF, Surge)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCE19_surge_new_{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW, Surge)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCE19_surge_ref_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    en_date = datetime(2019, 9, 10, 12)

    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_GLB_FCST_ETE2019_SURGE"
