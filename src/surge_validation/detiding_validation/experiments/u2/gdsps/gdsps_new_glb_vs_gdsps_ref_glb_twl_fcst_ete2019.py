...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-REF, TWL)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCE19_twl_ref_{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW, TWL)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCE19_twl_new_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_GLB_FCST_ETE2019_TWL"
