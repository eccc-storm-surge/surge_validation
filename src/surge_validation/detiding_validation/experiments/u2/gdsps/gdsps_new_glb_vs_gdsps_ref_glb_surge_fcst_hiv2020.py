...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/u2_seasonal_cycles/{label}")

    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-REF, Surge)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCH20_surge_ref_{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW, Surge)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCH20_surge_new_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_GLB_FCST_HIV2020_SURGE"

    logger = log_utils.get_logger(__name__)
    logger.info("Running %s for %s -- %s", EXP_ID, st_date, en_date)

    obs_file = Path("/home/olh001/Python/obs_to_grid_mapping/gdsps/gdsps_global_obs_v1.0.2.obs")
