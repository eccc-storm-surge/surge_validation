...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/u2/par/GDSPS/{label}")

    default_params.CUSTOM_COLOR_LIST = ["blue", "red", ]
    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-REF, TWL)",
            next((inp_data_root / "gdsps/par/na/").rglob(f"data_for_scoring_*gdsps*_twl_ref_*{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW, TWL)",
            next((inp_data_root / "gdsps/par/na/").rglob(f"data_for_scoring_*gdsps*_twl_new_*{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = f"GDSPS_NEW_vs_GDSPS_REF_FCST_PAR_TWL_EAST-WEST_{MAX_LEAD_H}h"
    st_date = datetime(2022, 5, 3, 12)
    en_date = datetime(2022, 6, 3, 12)
