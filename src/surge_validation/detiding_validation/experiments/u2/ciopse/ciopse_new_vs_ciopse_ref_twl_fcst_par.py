...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/u2/ciopse/par")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/u2/ciopse/par/{label}")
...
def main():
    ...
    st_date = datetime(2022, 5, 19, 0)
    en_date = datetime(2022, 5, 30, 0)
