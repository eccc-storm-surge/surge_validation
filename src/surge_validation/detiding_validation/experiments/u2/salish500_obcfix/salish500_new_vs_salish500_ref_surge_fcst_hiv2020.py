...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/u2/")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/u2_seasonal_cycles/salish500/obcfix/{label}")

    exp_id_to_path = OrderedDict([
        ("SALISH500 (FCST-REF, Surge)",
            next((inp_data_root / "ciopsw/ss500").rglob(f"data_for_scoring_*salish500*_ref_*hiv2020_blend_surge_{st_s}_{en_s}/surge*.dat"))),
        ("SALISH500 (FCST-NEW, Surge)",
            next((inp_data_root / "ciopsw/ss500").rglob(f"obcfix/data_for_scoring_*salish500*_new_*hiv2020_blend_surge_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2019, 12, 24, 0)
    en_date = datetime(2020, 2, 29, 00)
    EXP_ID = "SALISH500_NEW_vs_SALISH500_REF_FCST_HIV2020_BLEND_SURGE"
