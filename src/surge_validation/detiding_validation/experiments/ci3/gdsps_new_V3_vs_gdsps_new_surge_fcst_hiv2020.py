...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-NEW, Surge)",
            next((inp_data_root / "gdsps").rglob(f"data_for_scoring_gdsps_fcst_FCH20_surge_new_{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW-V3, Surge)",
            next((inp_data_root / "gdsps").rglob(f"data_for_scoring_gdsps_fcst_FCH20_surge_new_V3_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "GDSPS_NEW_V3_vs_GDSPS_NEW_FCST_HIV2020_SURGE"
