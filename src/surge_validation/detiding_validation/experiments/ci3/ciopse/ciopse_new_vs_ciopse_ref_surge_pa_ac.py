...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("CIOPSE (PA*-REF, Surge)",
            next((inp_data_root / "ciopse").rglob(f"data_for_scoring_*ciopse*r1.5_hindcastA*_surge_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSE (PA-NEW, Surge)",
            next((inp_data_root / "ciopse").rglob(f"data_for_scoring_*ciopse*ci3_pa*_surge_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2019, 3, 20, 1)
    en_date = datetime(2020, 3, 20, 1)
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_PA_AC_SURGE_BV3"
