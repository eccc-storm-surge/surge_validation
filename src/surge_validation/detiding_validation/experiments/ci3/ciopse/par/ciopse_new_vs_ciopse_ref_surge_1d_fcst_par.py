...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("FCST-REF, detided",
            next((inp_data_root / "ciopse/par/" ).rglob(f"data_for_scoring_*ciopse*_ref_*_surge_1d_{st_s}_{en_s}/surge*.dat"))),
        ("FCST-NEW, detided",
            next((inp_data_root / "ciopse/par/" ).rglob(f"data_for_scoring_*ciopse*_new_*_surge_1d_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    en_date = datetime(2021, 9, 29, 0)
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_FCST_PAR_Surge_daily-runs"
