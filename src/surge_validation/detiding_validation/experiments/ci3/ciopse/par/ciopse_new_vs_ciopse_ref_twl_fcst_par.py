...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/ci3/ciopse/par")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/ci3_seasonal_cycles/ciopse/{label}")

    exp_id_to_path = OrderedDict([
        ("CIOPSE (FCST-REF, TWL)",
            next((inp_data_root).rglob(f"data_for_scoring_*ciopse*_ref_*_twl_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSE (FCST-NEW, TWL)",
            next((inp_data_root).rglob(f"data_for_scoring_*ciopse*_new_*_twl_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    # 2021072700_2021092906
    st_date = datetime(2021, 7, 27, 0)
    en_date = datetime(2021, 9, 29, 6)
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_FCST_PAR_TWL"

    logger = log_utils.get_logger(__name__)
    logger.info("Running %s for %s -- %s", EXP_ID, st_date, en_date)
    station_dict = io_manager.read_station_dict_from_obs(Path("/home/olh001/Python/obs_to_grid_mapping/ciopse/ciopse_opt_v002.obs"))
