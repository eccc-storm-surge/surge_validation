...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("CIOPSE (PA*-REF, TWL)",
            next((inp_data_root / "ciopse").rglob(f"data_for_scoring_*ciopse*r1.5_hindcastA*_twl_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSE (PA-NEW, TWL)",
            next((inp_data_root / "ciopse").rglob(f"data_for_scoring_*ciopse*ci3_pa*_twl_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_PA_AC_TWL_BV3"
