...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-NEW-GLB, Surge)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCH20_surge_new_{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW-GLB-V4, Surge)",
            next((inp_data_root / "gdsps_glb").rglob(f"data_for_scoring_gdsps_fcst_FCH20_surge_new_V4_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "GDSPS_NEW_GLB_V4_vs_GDSPS_NEW_GLB_FCST_HIV2020_SURGE-cochin-manilla"
