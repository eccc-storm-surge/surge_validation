...
from cartopy.crs import LambertConformal, Robinson
...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/ci3/rdsps/par/")
    ...
    exp_id_to_path = OrderedDict([
        ("RDSPS_ref (FCST, Surge)",
            next((inp_data_root).rglob(f"data_for_scoring_rdsps_fcst_par_surge-only_ref_{st_s}_{en_s}/surge*.dat"))),
        ("RDSPS_new (FCST, Surge)",
            next((inp_data_root).rglob(f"data_for_scoring_rdsps_fcst_par_surge-only_new_{st_s}_{en_s}/surge*.dat"))),
    ])
    ...
    logger.info(f"split_seasons: {b2b_split_seasons}")
...
def main():
    ...
    st_date = datetime(2020, 10, 11, 0)
    en_date = datetime(2021, 10, 11, 0)

    EXP_ID = "RDSPS_NEW_vs_RDSPS_REF_FCST_PAR_SURGE-ONLY"

    logger = log_utils.get_logger(__name__)
    logger.info("Running %s for %s -- %s", EXP_ID, st_date, en_date)

    obs_file = Path("/home/olh001/Python/obs_to_grid_mapping/rdsps/rdsps_v002.obs")
    station_dict = io_manager.read_station_dict_from_obs(obs_file)

    # skip shediac bay
    station_dict = {k: v for k, v in station_dict.items() if k not in ["1805", ]}
