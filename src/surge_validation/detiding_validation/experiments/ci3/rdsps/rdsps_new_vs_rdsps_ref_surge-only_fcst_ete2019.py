...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("RDSPS_ref (FCST, Surge)",
            next((inp_data_root / "rdsps").rglob(f"data_for_scoring_rdsps_fcst_FCE19_surge-only_ref_{st_s}_{en_s}/surge*.dat"))),
        ("RDSPS_new (FCST, Surge)",
            next((inp_data_root / "rdsps").rglob(f"data_for_scoring_rdsps_fcst_FCE19_surge-only_new_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "RDSPS_NEW_vs_RDSPS_REF_FCST_ETE2019_SURGE-ONLY"
