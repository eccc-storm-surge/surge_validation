...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("CIOPSW (FCST-NEW, TWL)",
            next((inp_data_root / "ciopsw/par").rglob(f"data_for_scoring_*ciopsw*new_par_twl_*{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW, TWL)",
            next((inp_data_root / "gdsps/par/na/cmp_with_ciopsw/").rglob(f"data_for_scoring_*gdsps*_twl_new_*{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "GDSPS_NEW_vs_CIOPSW_NEW_FCST_PAR_TWL"
