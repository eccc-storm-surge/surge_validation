...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/ci3_seasonal_cycles/ete2021/GDSPS/{label}")

    exp_id_to_path = OrderedDict([
        ("RDSPS (FCST-REF, Surge)",
            next((inp_data_root / "rdsps/ete2021/").rglob(f"data_for_scoring_*rdsps*_surge_ref_*{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW, Surge)",
            next((inp_data_root / "gdsps/ete2021/na/").rglob(f"data_for_scoring_*gdsps*_surge_new_*{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    en_date = datetime(2021, 9, 10, 12)
    EXP_ID = "GDSPS_NEW_vs_RDSPS_REF_FCST_ETE2021_SURGE"
