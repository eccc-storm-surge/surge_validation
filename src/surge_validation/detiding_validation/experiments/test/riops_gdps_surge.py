"""
= RIOPS-GIOPS hindcast===============
Filtering tides in riops and obs for validation
"""
import logging
...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/fdu000/data/ppp3/verification/surge_validation/data/gdsps")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/ci3_seasonal_cycles/riops/{label}")

    exp_id_to_path = OrderedDict([
        ("GX33long (SS)", next(inp_data_root.rglob(f"data_for_scoring_*_{st_s}_{en_s}/surge_gdsps_pa_2019-2020.dat"))),
        ("RX22long (SS)", next(inp_data_root.rglob(f"data_for_scoring_*_{st_s}_{en_s}/surge_riops-gdsps_2019-2020.dat"))),
    ])
...
def main():
    st_date = datetime(2019, 7, 4, 9)
    en_date = datetime(2020, 7, 1, 3)
    EXP_ID = "RX22long_vs_GX-surge"
