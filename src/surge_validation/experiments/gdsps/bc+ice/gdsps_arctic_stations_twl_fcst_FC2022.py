...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (PA_NEW, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_arctic_stations_{st_s}_{en_s}").rglob("surge*.dat"))),
    ])

    exp_id_labels = list(exp_id_to_path)

    b2b_nhours = {
        lbl: 6 for lbl in exp_id_labels
    }
...
def main():
    # forecast
    EXP_ID = "GDSPS_ARCTIC_STATIONS_NEW_FC2022_TWL_Jul-Oct"
    st_date = datetime(2022, 7, 1, 0)
    en_date = datetime(2022, 11, 1, 0)
    ...
    station_dict = io_manager.read_station_dict_from_obs(
        "/home/olh001/Python/download_station_info/data/arctic_points_202301/new_stations_7.obs")
