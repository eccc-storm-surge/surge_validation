...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/gdsps/baroclinicity+ice")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/baroclinicity+ice/FC2022/GDSPS/{label}")
...
def main():
    ...
    en_date = datetime(2022, 7, 10, 0)
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_FCST_FC2022_SURGE_thicker_v1"
