...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/gdsps/baroclinicity+ice/par/r0.9/freq12h/filt_1/")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/baroclinicity+ice/PAR/GDSPS/{label}")

    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-REF, Surge)",
            next((inp_data_root / f"data_for_scoring_gdsps_fcst_na_surge_ref_{st_s}_{en_s}").rglob("surge*.dat"))),
        ("GDSPS (FCST-NEW, Surge)",
            next((inp_data_root / f"data_for_scoring_gdsps_fcst_na_surge_new_{st_s}_{en_s}").rglob("surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2023, 2, 14, 12)
    en_date = datetime(2023, 6, 26, 12)
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_FCST_PAR_SURGE_freq12h_filt_r0.9_v1"
