...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/gdsps/baroclinicity+ice/par/cd")
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (PSEUDO-REF, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_fcst_na_twl_ref_{st_s}_{en_s}").rglob("surge*.dat"))),
        ("GDSPS (PSEUDO-NEW, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_fcst_na_twl_new_{st_s}_{en_s}").rglob("surge*.dat"))),
    ])
...
def main():
    # forecast
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_PSEUDO_PAR_TWL-CD-COMPARE"
    st_date = datetime(2022, 6, 1, 0)
    en_date = datetime(2023, 6, 1, 0)
