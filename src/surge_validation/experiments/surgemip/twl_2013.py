...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/surgemip/data/loadprogs/SURGEMIP_TWL/loadprogs_2013010100_2013123123/output")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}_2013"
    img_dir = Path(f"data/plots/surgemip/{label}")

    exp_id_to_path = OrderedDict([
        ("ECCC_GDSPS (TWL)", inp_data_root / "data_for_scoring_ECCC_GDSPS_2013010100_2013123123/surge_ECCC_GDSPS.dat"),
        ("UND-ANL_GADCIRC-btp (TWL)", inp_data_root / "data_for_scoring_UND-ANL_GADCIRC-btp_2013010100_2013123123/surge_UND-ANL_GADCIRC-btp.dat"),
    ])
    ...
    print([s for s in stid_list if s.startswith("brunswick")])
...
def main():
    ...
    en_date = datetime(2013, 12, 31)

    EXP_ID = "SURGEMIP_TWL_GLOBAL"
    ...
    # debug
    station_dict = OrderedDict([(k, v) for k, v in station_dict.items() if k.startswith("brunswick")])
