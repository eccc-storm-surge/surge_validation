...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/u2_seasonal_cycles/ciopse/{label}")

    exp_id_to_path = OrderedDict([
        ("CIOPSE (FCST-REF, Surge)",
            next((inp_data_root / "ciopse").rglob(f"data_for_scoring_*ciopse*_ref_*_surge_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSE (FCST-NEW, Surge)",
            next((inp_data_root / "ciopse").rglob(f"data_for_scoring_*ciopse*_new_*_surge_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2019, 12, 23, 0)
    en_date = datetime(2020, 2, 29, 00)
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_FCST_HIV2020_SURGE"
