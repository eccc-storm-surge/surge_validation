...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/u2_seasonal_cycles/ciopsw/{label}")

    exp_id_to_path = OrderedDict([
        ("CIOPSW (FCST-REF, Surge)",
            next((inp_data_root / "ciopsw").rglob(f"data_for_scoring_*ciopsw*_ref_*blend_surge_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSW (FCST-NEW, Surge)",
            next((inp_data_root / "ciopsw").rglob(f"data_for_scoring_*ciopsw*_new_*blend_surge_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "CIOPSW_NEW_vs_CIOPSE_REF_FCST_HIV2020_BLEND_SURGE"
