...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/u2/ciopse/pa/")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/u2_seasonal_cycles/ciopse/pa/{label}")

    exp_id_to_path = OrderedDict([
        ("CIOPSE (PA-REF, Surge)", f"data_for_scoring_*ciopse*_ref_*_surge_noflt_{st_s}_{en_s}/surge*.dat"),
        ("CIOPSE (PA-NEW-noflt-fix, Surge)", f"data_for_scoring_*ciopse*_new_*_surge_fix_noflt_{st_s}_{en_s}/surge*.dat"),
    ])

    for k, pth in exp_id_to_path.items():
        try:
            pth = next(inp_data_root.rglob(pth))
            exp_id_to_path[k] = pth
        except StopIteration as err:
            raise IOError(f"No match for {pth} in {inp_data_root}")
...
def main():
    ...
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_PA_HIV2020_SURGE-noflt-fix"
