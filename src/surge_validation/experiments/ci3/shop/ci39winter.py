...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("SHOP (FCST, TWL)",
            next((inp_data_root / "shop").rglob(f"data_for_scoring_SHOP_CI39_winter_TWL_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2019, 12, 21, 0)
    en_date = datetime(2020, 3, 10, 12)

    EXP_ID = "SHOP_TWL_CI39_V2"

    logger = log_utils.get_logger(__name__)
    logger.info("Running %s for %s -- %s", EXP_ID, st_date, en_date)

    station_dict = io_manager.read_station_dict_from_obs(
        "/home/olh001/Python/obs_to_grid_mapping/shop/shop_ci3_wl_v1.0.0.obs")
