...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/dev/loadprogs_python/data/riops/")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/ci3_seasonal_cycles/riops/{label}_{st_s}-{en_s}")

    print("Contents of the input root folder: ")
    for d in inp_data_root.iterdir():
        print(d)

    exp_id_to_path = OrderedDict([
        ("RIOPS (Surge)",
            next(inp_data_root.rglob(f"*data_for_scoring_RX21LONG*{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2019, 3, 6, 0)
    en_date = datetime(2020, 3, 5, 0)
    EXP_ID = "RIOPS_TEST"
