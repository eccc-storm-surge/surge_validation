...
def fc(station_dict=default_params.station_dict,
    ...
    img_dir = Path(f"data/plots/ci3_seasonal_cycles/wcps_cc1/{label}")

    exp_id_to_path = OrderedDict([
        ("GSL (FCST-REF, Surge)",
            next((inp_data_root / "gsl").rglob(f"data_for_scoring_*GSL*_surge_{st_s}_{en_s}/surge*.dat"))),
        ("WCPS_CC1 (FCST-NEW, Surge)",
            "/fs/site3/eccc/cmd/e/olh001/Python_hub/dev/loadprogs_python/ci3/wcps_cc1/data_for_scoring_WCPS_CC1_E19-daily_surge_2019070100_2019083100/surge_WCPS_CC1_E19-daily_surge.dat"),
    ])

    exp_id_labels = list(exp_id_to_path)

    b2b_nhours = {
        "GSL (FCST-REF, Surge)": 13,
        "WCPS_CC1 (FCST-NEW, Surge)": 25
    }
...
def main():
    ...
    EXP_ID = "WCPS_CC1_vs_GSL_FCST_ETE2019_SURGE-24hourly-test"
