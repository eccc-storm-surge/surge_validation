...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GSL (FCST-REF, TWL)",
            next((inp_data_root / "gsl").rglob(f"data_for_scoring_*GSL*_twl_{st_s}_{en_s}/surge*.dat"))),
        ("WCPS_CC1 (FCST-NEW, TWL)",
            next((inp_data_root / "wcps_cc1").rglob(f"data_for_scoring_*WCPS_CC1*_twl_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "WCPS_CC1_vs_GSL_FCST_HIV2020_TWL"
