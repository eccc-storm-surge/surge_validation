...
def main():
    ...
    EXP_ID = "RESPS_NEW_vs_RESPS_REF_FCST_PAR_SURGE"
    st_date = datetime(2020, 10, 9, 0)
    en_date = datetime(2021, 10, 9, 0)
...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/ci3/resps/par")
    ...
    exp_id_to_path = OrderedDict([
        ("RESPS (FCST-REF, Surge)",
            next((inp_data_root).rglob(f"data_for_scoring_*resps*_surge_ref_*{st_s}_{en_s}/surge*.dat"))),
        ("RESPS (FCST-NEW, Surge)",
            next((inp_data_root).rglob(f"data_for_scoring_*resps*_surge_new_*{st_s}_{en_s}/surge*.dat"))),
    ])
    ...
    exclude_ids = ["491", "1805"]
    stid_list = [stid for stid in stid_list if stid not in exclude_ids]
