...
def main():
    ...
    EXP_ID = "RESPS_NEW_vs_RESPS_REF_FCST_PAR_TWL"
    st_date = datetime(2021, 5, 26, 12)
...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("RESPS (FCST-REF, TWL)",
            next((inp_data_root).rglob(f"data_for_scoring_*resps*_twl_ref_*{st_s}_{en_s}/surge*.dat"))),
        ("RESPS (FCST-NEW, TWL)",
            next((inp_data_root).rglob(f"data_for_scoring_*resps*_twl_new_*{st_s}_{en_s}/surge*.dat"))),
    ])
