...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("CIOPSE (FCST-REF, Detided)",
            next((inp_data_root / "ciopse/par/" ).rglob(f"data_for_scoring_*ciopse*_ref_*_surge_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSE (FCST-NEW, Detided)",
            next((inp_data_root / "ciopse/par/").rglob(f"data_for_scoring_*ciopse*_new_*_surge_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_FCST_PAR_Surge"
