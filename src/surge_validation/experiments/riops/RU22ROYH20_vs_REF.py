...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("REF (Surge)", inp_data_root / f"data_for_scoring_RU22REFH20_{st_s}_{en_s}" / "surge_RU22REFH20.dat"),
        ("RU22ROYH20 (Surge)", inp_data_root / f"data_for_scoring_RU22ROYH20_{st_s}_{en_s}" / "surge_RU22ROYH20.dat"),
    ])
    ...
    b2b_split_seasons = {
        f"{t:%b}": (t.month, ) for t in [datetime(2001, m, 1) for m in [1, 2, 12]]
    }
