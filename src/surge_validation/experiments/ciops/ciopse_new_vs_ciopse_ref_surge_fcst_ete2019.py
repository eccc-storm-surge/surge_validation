...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root_ref = Path("/home/yls000/CIOPS/load_progs_python_experiments/data/ciopse/f/C1EFC21E19V1")
    inp_data_root_new = Path("/home/yls000/CIOPS/load_progs_python_experiments/data/ciopse/f/C1E211E19V2")
    ...
    img_dir = Path(f"data/plots/211_final_cycles/ciopse/f/{label}")

    exp_id_to_path = OrderedDict([
        ("CIOPSE (FCST-REF, Surge)",
            next(inp_data_root_ref.rglob(f"data_for_scoring_*ciopse*_ref_*_surge_{st_s}_{en_s}/surge*.dat"))),
        ("CIOPSE (FCST-NEW, Surge)",
            next(inp_data_root_new.rglob(f"data_for_scoring_*ciopse*_new_*_surge_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    st_date = datetime(2019, 7, 8, 0)
    en_date = datetime(2019, 8, 30, 00)
    EXP_ID = "CIOPSE_NEW_vs_CIOPSE_REF_FCST_ETE2019_SURGE"
