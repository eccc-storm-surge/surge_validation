...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root_ref = Path("/home/jot000/CIOPS/load_progs_python_experiments/data/ciops_ic4/ciopsw/ss500/pa/FC")
    inp_data_root_new = Path("/home/jot000/CIOPS/load_progs_python_experiments/data/ciops_ic4/ciopsw/ss500/pa/FC")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/ci4/ciopsw/FC/salish500/pa/{label}")

    exp_id_to_path = OrderedDict([
        ("SALISH500 (PA-REF, TWL)",
            next(inp_data_root_ref.rglob(f"data_for_scoring_*salish500*_ref_*twl_wPTC_{st_s}_{en_s}/surge*.dat"))),
        ("SALISH500 (PA-NEW, TWL)",
            next(inp_data_root_new.rglob(f"data_for_scoring_*salish500*_new_*twl_wPTC_{st_s}_{en_s}/surge*.dat"))),
    ])
    ...
    options = {
        ...
        "score_map_figsize": (14, 12),
        "score_map_marker_size": 40,
...
def main():
    ...
    st_date = datetime(2021, 10, 19, 1)
    en_date = datetime(2022, 5, 31, 0)
    EXP_ID = "C2WFC23H22V1_vs_C2WFC22H22V1_TWL_wPTC"
