...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (PROG-REF, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_prog_na_twl_nodebias_ref_{st_s}_{en_s}").rglob("surge*.dat"))),
        ("GDSPS (PROG-NEW, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_prog_na_twl_nodebias_new_{st_s}_{en_s}").rglob("surge*.dat"))),
    ])
    ...
    options = {
        ...
        "score_map_figsize": (18.5, 8),
        "score_map_marker_size": 100,
        "score_map_colorbar_fraction": "6%",
...
def main():
    # forecast
    EXP_ID = "GDSPS_200_vs_GDSPS_210_PROG_PAR_TWL_NODEBIAS"
