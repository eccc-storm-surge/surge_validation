...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (PROG-REF, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_prog_na_twl_ref_{st_s}_{en_s}").rglob("surge*.dat"))),
        ("GDSPS (PROG-NEW, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_prog_na_twl_new_{st_s}_{en_s}").rglob("surge*.dat"))),
    ])
