...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/olh001/Python/loadprogs_python_experiments/data/gdsps/ci4-final-cycles/prog/")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/ci4/GDSPS/final_cycles_2021110112-2022053112/{label}")

    exp_id_to_path = OrderedDict([
        ("GDSPS (FCST-REF-GLB, TWL)",
            next(inp_data_root.rglob(f"data_for_scoring_gdsps_prog_global_twl_ref_{st_s}_{en_s}/surge*.dat"))),
        ("GDSPS (FCST-NEW-GLB, TWL)",
            next(inp_data_root.rglob(f"data_for_scoring_gdsps_prog_global_twl_new_{st_s}_{en_s}/surge*.dat"))),
    ])
...
def main():
    ...
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_PROG_TWL_FS22_GLOBAL"
