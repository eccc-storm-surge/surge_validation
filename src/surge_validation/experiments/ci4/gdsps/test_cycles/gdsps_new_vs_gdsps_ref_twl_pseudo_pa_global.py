...
def fc(station_dict=default_params.station_dict,
    ...
    exp_id_to_path = OrderedDict([
        ("GDSPS (PSEUDO-REF, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_pa_global_twl_ref_{st_s}_{en_s}").rglob("surge*.dat"))),
        ("GDSPS (PSEUDO-NEW, TWL)",
            next((inp_data_root / f"data_for_scoring_gdsps_pa_global_twl_new_{st_s}_{en_s}").rglob("surge*.dat"))),
    ])
...
def main():
    # forecast
    EXP_ID = "GDSPS_NEW_vs_GDSPS_REF_PSEUDO_TEST-CYCLES_TWL-GLOBAL"
    ...
    station_dict = io_manager.read_station_dict_from_obs(
        "/home/olh001/Python/download_station_info/data/ushlc/2022010100-2023060100/info-idx.obs")
