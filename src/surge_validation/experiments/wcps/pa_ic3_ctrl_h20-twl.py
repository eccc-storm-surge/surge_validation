...
config["data_dir"] = "/home/olh001/.local/share/cartopy"
...
def fc(station_dict=default_params.station_dict,
    ...
    inp_data_root = Path("/home/fdu000/data/ppp3/verification/surge_validation/data")

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    label = f"{exp_id}_{st_s}_{en_s}"
    img_dir = Path(f"data/plots/ci3_seasonal_cycles/wcps/{label}")

    exp_id_to_path = OrderedDict([
        ("(FCST-REF, Surge)",
            next((inp_data_root / "wcps").rglob(f"data_for_scoring_CTRLOPS-twl_{st_s}_{en_s}/surge*.dat"))),
        ("(PAV3-NEW, Surge)",
            next((inp_data_root / "wcps").rglob(f"data_for_scoring_C2300H20V3-twl_{st_s}_{en_s}/surge*.dat"))),
        ("(PAV4-NEW, Surge)",
            next((inp_data_root / "wcps").rglob(f"data_for_scoring_C2300H20V4-twl_{st_s}_{en_s}/surge*.dat"))),
        ("(PAV4b-NEW, Surge)",
            next((inp_data_root / "wcps").rglob(f"data_for_scoring_C2300H20V4b-twl_{st_s}_{en_s}/surge*.dat"))),
    ])
