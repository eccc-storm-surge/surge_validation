...
def fc(station_dict=default_params.station_dict, st_date=None, en_date=None, member_id=""):
    ...
    label = f"{EXP_ID}_{member_id}"
    # img_dir = Path(f"data/plots/{label}_{datetime.utcnow():%Y%m%d%H%M}")
    img_dir = Path(f"data/plots/{label}")

    inp_data_root = "/home/olh001/Python/loadprogs_python/data/migration_2019_par_cycles/resps/"

    st_s = f"{st_date:%Y%m%d%H}"
    en_s = f"{en_date:%Y%m%d%H}"

    swl_path_old = inp_data_root + f"data_for_scoring_resps_120-ops_{st_s}_{en_s}/surge_resps_120-ops.dat"
    swl_path_new = inp_data_root + f"data_for_scoring_resps_130-par_{st_s}_{en_s}/surge_resps_130-par.dat"
