...
def _fname_to_origin_date(fn):
    return datetime.strptime(fn.split("_")[0], "%Y%m%d%H").replace(tzinfo=timezone.utc)


def export_to_txt(src_dir: Path, dst_dir: Path, data_query=None):
    ...
    for f_in in src_dir.iterdir():
        ...
        exp_t = _fname_to_origin_date(f_in.name)
...
def main():
    ...
    end_time = datetime(2019, 4, 26, tzinfo=timezone.utc)
    ...
    label_to_data_dir = OrderedDict([
        ...
        ("WT", root_dir / "tides_for_loadprogs"),
