...
def plot_cotidal_range_for_member(member_id, ax, data, amp_clevs=np.arange(0, 1.5, 0.05), show_cb=False, plot_phase=True):
...
def plot_cotidal_range(lons, lats, constituents: dict, img_dir: Path = Path("data/plots"), sel_constituent_names=("M2",)):
    ...
    gs = GridSpec(nrows, ncols, hspace=0.0, wspace=-0.2)
    ...
    for cn in sel_constituent_names:
        ...
        for i, (member_id, constit_fields) in enumerate(constituents.items()):
            show_cb = (i == len(constituents) - 1)

            to_plot = {
                "coords": (lons, lats),
                "amp": constit_fields[cn]["amp"],
...
def test():
    ...
    constituents = {
        member_id: get_constituents(data[member_index, :, :], lat=lats[mask], dt_hours=1, nprocs=1)
